...
x = -xL-sqrt(rL*(1-rL)) % segno - -> reattanza positiva
LX = (x*Z0)/(2*pi*f) % elemento jX -> induttanza 87.616 nH
...
PLa = 1-(abs(Gamma2))^2 % potenza attiva al carico 85.71%

b = -sqrt(rL*(1-rL))/rL % segno - -> suscettanza negativa
LB = -Z0/(2*pi*f*b) % elemento jB -> induttanza 9.7462 nH
