...
%thetainc=0; % incidenza normale
%thetainc=pi/6; % incidenza obliqua
%thetainc=pi/3; % angolo di Brewster
thetainc=pi/5; % angolo critico

%% mezzo 1
%epsr1=1; mur1=1; % aria
epsr1=3; mur1=1; % dielettrico

%epsr1=2.2*(1-1i*1.2e-4); mur1=1; % dielettrico con perdite
...
%% mezzo 2
%epsr2=3; mur2=1; % dielettrico senza perdite
%epsr2=3-0.3*i; mur2=1; % dielettrico con perdite
epsr2=1; mur2=1; % aria

%epsr2=2.2*(1-1i*1.2e-4); mur2=1; % dielettrico con perdite
% epsr2=6-0.06i; mur2=1; % PTFE
% epsr2=4; mur2=1; % SiO2
% epsr2=9; mur2=1; % Alumina
% epsr2=3.9-0.3i; mur2=1; % PTFE
% sigma=5.8e7; epsr2=1-1i*sigma/(omega*eps0);  mur2=1;% Cu
% sigma=1.4e7;   mur2=1; % Brass
% sigma=6.3e7; epsr2=1-1i*sigma/(omega*eps0);  mur2=1;% Ag
...
% epsr2=-1e20i;  mur2=1;% PEC
% epsr2=10-20i; mur2=1;% H2O @ 60GHz 20°C
% epsr2=20-30i; mur2=1;% H2O @ 60GHz 20°C
% epsr2=1+1/(1+1i-(1/2)^2); mur2=1;

%omega0=2*pi*f; epsr2=-4i*omega*omega0/(omega^2+omega0^2); mur2=-2i;

eta2=eta0*sqrt(mur2/epsr2); % impedenza caratteristica
k2=k0*(-1i)*sqrt(-mur2*epsr2); % costante di propagazione
%delta=(1-1i)/k2;
sinthetatra=k1*sin(thetainc)/k2; costhetatra=-1i*sqrt(sinthetatra^2-1);
thetatra=acos(costhetatra); % angolo di trasmissione
...
% for nt=1:Nt
%    pcolor(X,Z,real(E(:,:,2)*exp(1i*2*pi*(nt-1)/Nt))); shading flat;axis equal tight; colormap jet; caxis(cax); colorbar, title('e_y(t) (V/m)'), xlabel('x (m)'), ylabel('z (m)')
%     Mov(nt)=getframe;
% end
% movie(Mov,10,2*Nt)
figure(3)
